...
// Suma parcial para cada hilo
void suma_parcial(const std::vector<int>& datos, size_t inicio, size_t fin, long long& resultado) {
    long long suma = 0;
...
int main() {
    ...
    auto duracion_sec = std::chrono::duration_cast<std::chrono::milliseconds>(fin_sec - inicio_sec).count();
    ...
    auto duracion_par = std::chrono::duration_cast<std::chrono::milliseconds>(fin_par - inicio_par).count();
